...
NTSTATUS nullhook::hook_handler(PVOID called_param)
{
	...
	else if (instructions->draw_line == TRUE) 
	{
		HDC hdc = NtUserGetDC(NULL);
		if (!hdc)
			return STATUS_UNSUCCESSFUL;
		...
		DrawLine(hdc, instructions->x, instructions->y, instructions->dx, instructions->dy, brush, instructions->t, instructions->x_step, instructions->y_step);
		NtUserReleaseDC(hdc);
		NtGdiDeleteObjectApp(brush);
	}
...
INT nullhook::DrawLine(HDC hDC, int x, int y, int dx, int dy, HBRUSH hbr, int thickness, int x_step, int y_step) {
	...
	//for (i = x, j = y; i != dx || j != dy; i = i + x_step, j = j + y_step) {
	//	NtGdiPatBlt(hDC, i, j, thickness, thickness, PATCOPY);
	//}
	if (x * x < y * y) {
		if (y > dy) {
			//x_step is a!
